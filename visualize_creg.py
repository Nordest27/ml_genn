"""
compare_creg_groups.py
Compares CSVs that share a common name *prefix* but differ by a trailing
"_creg_0XX" suffix, e.g.:
    pacman_training_weight_dist_prop_creg_05.csv
    pacman_training_weight_dist_prop_creg_075.csv
    pacman_training_weight_dist_prop_creg_1.csv
    pacman_training_other_method_creg_025.csv
    pacman_training_other_method_creg_05.csv

Layout: one ROW per group, one COLUMN per metric panel.
Within each cell only that group's creg curves are drawn, so shades
never mix between groups. Creg values are labeled directly at the
...
OUT_NAME = "comparison_creg_groups.png"
WINDOW_EP = 250  # episode-window size for smoothing
DPI = 200

# Shade range: lightest (smallest c) → darkest (largest c).
# Wider range = easier to tell c values apart.
SHADE_LIGHTEST = 0.15
SHADE_DARKEST = 1.00

# Line width range: thinnest for smallest c, thickest for largest c.
LW_MIN = 0.8
LW_MAX = 2.2
...
# creg shading (light->dark per c) is layered on top of whatever base
...
PATTERN_COLORS: list[tuple[str, str]] = [
    ...
    ("asfadsfsadfasdf",             "#DD8452"),  # any other creg_* variant -> base orange
...
CREG_SUFFIX_RE = re.compile(r"^(.*)_creg_(\d+)$")

# Matches a "[<chart title>]" tag anywhere in the filename stem, e.g.
# "pacman_training_weight_dist_prop[Creg Sweep]_creg_05" -> tag
# "Creg Sweep". Used to route a file into its own comparison chart,
# independent of the "_creg_0XX" suffix. Files with no tag fall into the
...
def parse_creg_str(creg_str):
    """
    A leading zero (with more than one digit) means the digits are read as
    decimal places, i.e. as if a decimal point were inserted right after
    that leading zero. Anything else (a single digit, or a string that
    doesn't start with '0') is just the literal integer value.

    Examples:
        09   -> 0.9    (leading zero, len>1 -> fraction)
        05   -> 0.5
        075  -> 0.75
        025  -> 0.25
        01   -> 0.1
        001  -> 0.01
        1    -> 1.0    (no leading zero -> literal)
        10   -> 10.0   (no leading zero -> literal)
        0    -> 0.0    (single digit -> literal)
    """
    if creg_str[0] != "0" or len(creg_str) == 1:
        return float(int(creg_str))

    return int(creg_str) / (10 ** (len(creg_str) - 1))
...
def group_files_by_creg(files):
    """Group CSV paths first by chart tag ('[TEXT]', anywhere in the
    filename), then by prefix/creg as before (tag removed before suffix
    ...
    for f in files:
        ...
        m = CREG_SUFFIX_RE.match(stem)
        if not m:
            print(f"  -  skipping {f!r} (doesn't match '<prefix>_creg_<N>')")
            continue
        prefix, creg_str = m.group(1), m.group(2)
        lam = parse_creg_str(creg_str)
...
def render_comparison(chart_key, groups):
    ...
    out_name = (
        ...
        else f"comparison_creg_groups_{chart_slug}.png"
    )
    ...
    for panel_idx, panel in enumerate(PANELS):
        ...
        for group_idx, (prefix, lam_dfs) in enumerate(loaded.items()):
            ...
            styles = creg_styles(lam_sorted, base_color)
    ...
    fig.text(
        0.5,
        0.005,
        "Lighter, thinner lines = smaller c   ·   Darker, thicker lines = larger c",
        ha="center",
        va="bottom",
        fontsize=8,
        style="italic",
...
def main():
    ...
    charts = group_files_by_creg(files)
    if not charts:
        print("No files matched the '<prefix>_creg_<N>' naming pattern.")
        return
    ...
    for chart_key in sorted(charts.keys()):
        ...
        for prefix, lam_map in sorted(groups.items()):
            lam_list = ", ".join(f"c={lam:g}" for lam in sorted(lam_map.keys()))
            print(f"  Group {prefix!r}: {len(lam_map)} creg value(s) → {lam_list}")
